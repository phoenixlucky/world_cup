...
const labels: { key: keyof Weights; label: string; color: string }[] = [
  ...
  { key: 'worldCupPerf', label: '🏟️ 世界杯表现', color: '#ef4444' },
...
export function WeightPanel({ weights, onChange }: Props) {
  ...
  return (
    <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 sm:p-6">
      ...
      <button
        onClick={() =>
          onChange({ rank: 14, marketVal: 9, goals: 7, attackDefense: 18, opponentStrength: 10, wins: 14, form: 9, luck: 5, hostBonus: 5, worldCupPerf: 10 })
        }
